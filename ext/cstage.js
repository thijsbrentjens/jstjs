...
var cached_data_folder='estat_data';
var data_set='hlth_cd_acdr2';

cached_data_folder='estat_data/data_old';
data_set='hlth_cd_acdr';

var stage={};
stage.basename='localhost';
var theLayout;
var map;
var geocoder;
/** @constructor */
...
stage.tileUrl=function(coord,zoom)
{
	...
	var style="tjs_color";
	var url = stage.host+"cache.php?service=WMS&version=1.1.0&request=GetMap&styles="+style+"&width=256&height=256&srs=EPSG:4326&format=image%2Fpng";
...
stage.getGDASLink=function(icd)
{
	return "http://"+stage.basename+"/to-gdas/sdmx?framework_url=http%3A%2F%2F"+stage.basename+"%2Fgeoserver%2Ftjs%3Fservice%3DTJS%26version%3D1.0.0%26request%3DDescribeFrameworks&dataset_url=http%3A%2F%2F"+stage.basename+"%2F"+cached_data_folder+"%2F"+icd+".xml";
}
	...
	$.get('tjs_proxy.php',{'p':p},function(data){
		...
		stage.jqXHRs['variableValues']=$.get('wfs_proxy.php',{'p':'?service=WFS&version=1.0.0&request=GetFeature&typeName=__temp:'+stage.layerTable+'&propertyName=ICC,NUTS_CODE,NUTS_LABEL,'+dimensions.join(',')+',OBJECTID&outputFormat=application/json'},function(data)
		{
			...
			for (var i=0; i<data.features.length;++i)
			{
				...
				var iccode=parseInt(prop['OBJECTID']);
...
stage.setVariablePaginationWidget=function(control_id,fun)
{
	...
	if (cache.pagination===undefined)
	{
		...
		$.get(stage.getGDASLink(icd),function(data){
			var years=[];
			$(data).find("ColumnSet Column[name^='dim_']").each(function(){
